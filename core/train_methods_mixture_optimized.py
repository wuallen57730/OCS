"""
Optimized Mixture Training Methods
...
def compute_per_sample_grads_vectorized(model, criterion, data, target, task_id):
    ...
    This replaces the autograd_hacks/compute_and_flatten_example_grads approach
    with more efficient vectorization.
...
def classwise_fair_selection(task, cand_target, sorted_index, num_per_label, config, is_shuffle=True):
    """Class-balanced sample selection for Mixture dataset"""
        ...
        if config['select_type'] in coreset_methods:
            ...
            if config['select_type'] == 'coreset':
                pick, _ = bc.build_with_representer_proxy_batch(
                    ...
                    num_examples_per_task, kernel_fn, data_weights=None, cache_kernel=True,
            ...
            if config['select_type'] == 'coreset':
                if is_last_step:
                    selected_pick, _ = bc.build_with_representer_proxy_batch(
                        ...
                        config['batch_size'], kernel_fn, data_weights=None, cache_kernel=True,
                        start_size=1, inner_reg=1e-3)
            else:
