"""
MNIST OCS Training - Vectorized Version
Uses torch.func's vmap + grad for per-sample gradients computation
"""
...
from core.train_methods_mnist_vectorized import train_task_sequentially, eval_single_epoch
...
if 'rot-mnist' in DATASET:
    config = {'exp_name': '-ocs-vectorized',
              # ---COMMON----
...
def main():
    print('Started the trial >>', TRIAL_ID, 'for experiment 1 (VECTORIZED)')
    print(config)
    # init and save
    ...
    for task in range(1, config['num_tasks'] + 1):
        ...
        print('---- Task {} (OCS VECTORIZED) ----'.format(task))
